...
struct stateRec{
    stateRec(int prodID, int SeqPos, int origin, stateRec* Prev, stateRec* Child):productionID(prodID), SeqPosition(SeqPos), originPos(origin), prev(Prev), child(Child){}
    int productionID;
    int SeqPosition;
    int originPos;
    stateRec* prev;
...
struct testParser{
    ...
    //    grammar.push_back(production(fTerm+pmSEQ, " World!"));
        ...
        grammar.push_back(production(pmSEQ, (vector<int>){2, 3, 4}));
        grammar.push_back(production(fTerm+pmSEQ, "{"));
        grammar.push_back(production(fTerm+pmSEQ, "X"));
        grammar.push_back(production(fTerm+pmSEQ, "} "));
    };
    ...
    void addProductionToStateSet(int crntPos, int productionID, int SeqPos, int origin, stateRec* prev=0, stateRec* child=0){
        printf("############ ADD %i at %i (prev=%p): ", productionID, crntPos, prev);
        ...
        if(ProdType == pmSEQ || ProdType == pmREP){
            ...
            SSets[crntPos].stateRecs.push_back(new stateRec(productionID, SeqPos, origin, prev, child));
            printf("ADDED %i, %p\n", SeqPos, prev);
    ...
    bool complete(stateRec* SRec, int crntPos){
        printf("        COMPLETER... %p", SRec);
        stateSets &SSet=SSets[SRec->originPos];
        for(int crntRec=0; crntRec < SSet.stateRecs.size(); ++crntRec){
            stateRec* backSRec=SSet.stateRecs[crntRec];

            auto flags=grammar[backSRec->productionID].flags;
            int ProdType  = flags & maskProdMode;
            int isTerminal= flags & maskTermMode;
            vector<int>& items=grammar[backSRec->productionID].items;
            printf("************* SRec->child=%p, %p\n", SRec->child, backSRec);
            if(ProdType==pmREP){
