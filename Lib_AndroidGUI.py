...
def use(objects, buildSpec, tags, platform):
    CODE="""
struct CanvasView: ctxTag="AndroidGUI" Platform='Android' Lang='Java' LibReq="AndroidGUI" implMode="inherit:View" {
    me Paint: paint 
    me Path: path 
    me double: cur_x
    me double: cur_y

    me void: CanvasView(me Context: context ) <- <%    {
        super(context);
    }%>

    me void: onDraw(me Canvas: canvas) <- <%    {
        drawAppArea_cb(canvas);
    }%>
}
...
//    me void: fetchAreaToBeDrawn(me GUI_rect: area) <- <%!;%>
    ...
    me GUI_item: newCanvas() <- <%!%Gnew CanvasView(this)%>
    ...
    me GUI_item: newScrollingWindow() <- <%!%Gnew ScrollView()%>
...
struct GUI_ctxt: ctxTag="AndroidGUI" Platform='Android' Lang='Java' LibReq="AndroidGUI" implMode="inherit:JPanel" {
    ...
    me void: moveTo(me double: x, me double: y) <- <%!path.moveTo(%1, %2)%>
    me void: lineTo(me double: x, me double: y) <- <%!path.lineTo(%1, %2)%>
    ...
    me void: strokeNow() <- <%!canvas.drawPath(path, paint)%>
}
    """
