...
#CoFactualEL  = "(" + Group(ParseElement + "<=>" + Group(OneOrMore(SetFieldStmt + Literal(';').suppress())))  + ")" ("CoFactualEL")
#AlternateEl  = "[" + Word(alphas) + Group(OneOrMore(ParseElement + Optional("|").suppress())) + "]" ("AlternateEl")
fieldDefs = Literal("{").suppress() + Group(ZeroOrMore(fieldDef))+ Literal("}").suppress()("fieldDefs")
anonModel = fieldDefs ("anonModel")  #(CoFactualEL | AlternateEl | fieldDefs) ("anonModel")
